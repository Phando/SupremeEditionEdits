...
//--------------------------------------------------------------------------------------------------------------------
  ...
  WF.SampleStripe { //the remix player waveform with position indicator
	...
	Column {
		...
		Repeater {
			...
			Sample {
				...
				samplePropertyPath: slot.columnPropertyPath + ".rows." +  (firstRowInCurrentPage + index);
				//anchors.topMargin: 0		  
				anchors.bottomMargin: 0
...
//--------------------------------------------------------------------------------------------------------------------
  ...
  StepSequencer {
	active: sequencerMode.value && sequencerSlot.value == (index+1)
	deckId: slot.deckId
  ...
  Rectangle {
	...
	visible: sequencerMode.value && sequencerSlot.value == (index+1) //&& bottomInfoDetails.state != "FX" && bottomInfoDetails.state != "MIDI" per a que no es vegi si no s'esta controlant si es controla els FX3/4 o la midi
  }
