...
//-------------------------------------------------------------------------------------------------------------------
    ...
    //Browser Navigation
    MappingProperty { id: browserScroll; path: propertiesPath + ".browserScroll";
      onValueChanged: {
          if (browserScroll.value != 0) {
              var newValue = utils.clamp(qBrowser.currentIndex + browserScroll.value, 0, contentList.count - 1);
              // center selection if user is _fast scrolling_ but we're at the _beginning_ or _end_ of the list
              if (browserScroll.value >= browserRows.value) {
              ...
              browserScroll.value = 0
              doScrolling()
          }
      }
    }

    MappingProperty { id: browserExitNode; path: propertiesPath + ".browserExitNode";
      onValueChanged: {
          if (browserExitNode.value) {
              qBrowser.exitNode()
              //browser.relocateCurrentIndex()
              browserExitNode.value = false;
          }
      }
    }
