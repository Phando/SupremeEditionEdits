...
//------------------------------------------------------------------------------------------------------------------
    ...
    Blinker { name: "GridBlinker"; cycle: 250; defaultBrightness: dimmed; blinkBrightness: bright }
    ButtonScriptAdapter {
        name: "gridButton"
        brightness: holdGrid.value ? bright : dimmed //"GridBlinker"
        onPress: {
    ...
    WiresGroup {
        ...
        WiresGroup {
            ...
            Wire { from: "%surface%.grid_adjust.value"; to: "gridButton" }
            Wire { from: "%surface%.grid_adjust.led"; to: "GridBlinker"; enabled: !gridPressed && !holdGrid.value }
            Wire { from: "%surface%.grid_adjust"; to: ButtonScriptAdapter { brightness: bright } enabled: holdGrid.value || gridPressed }

            //Wire { from: "%surface%.grid_adjust"; to: HoldPropertyAdapter { path: holdGrid.path; value: true } enabled: !shift }
            //Wire { from: "%surface%.grid_adjust"; to: TogglePropertyAdapter { path: editMode.path; value: true } enabled: shift }
        }
        Wire { from: "%surface%.grid_adjust"; to: TogglePropertyAdapter { path: scratchOnTouch.path; value: true; defaultValue: false } enabled: shift }
    }
