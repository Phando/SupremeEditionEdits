...
//------------------------------------------------------------------------------------------------------------------
    ...
    //Hold button State
    MappingPropertyDescriptor { id: holdCapture; path: propertiesPath + ".holdCapture"; type: MappingPropertyDescriptor.Boolean; value: false }
...
//------------------------------------------------------------------------------------------------------------------
    ...
    function defaultFooterPage() {
        if (hasFXFooter.value) {
            ...
            footerPage.value = FooterPage.pitch;
        }
...
//------------------------------------------------------------------------------------------------------------------
    ...
    S8Faders {
        name: "faders"
        active: footerControlled //footerId == deckId //not working?
        deckId: deck.deckId
        surface: deck.surface
    }
    ...
    Wire { from: "%surface%.edit"; to: TogglePropertyAdapter { path: editMode.path; value: EditMode.full} enabled: active && hasEditMode && screenView.value == ScreenView.deck}
    ...
    function validatePadsMode(padsMode) {
        switch (padsMode) {
            ...
            case PadsMode.disabled:
                return true //pads can always be disabled no matter which deckType there is, otherwise an infinite loop can be generated and Traktor crashes
        }
    }
